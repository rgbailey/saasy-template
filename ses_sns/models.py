...
from ses_sns.utils import (
    get_permanent_bounced_emails_from_bounce_obj,
    get_emails_from_complaint_obj,
)

logger = logging.getLogger(__name__)

NOTIFICATION_STATUSES = namedtuple(
    "NOTIFICATION_STATUSES", "new processed failed"
)._make(range(3))
...
class SNSNotification(models.Model):
    ...
    state = models.SmallIntegerField(
        default=NOTIFICATION_STATUSES.new, choices=STATE_CHOICES, db_index=True
    )
        ...
        try:
            if self.data.get("Type") == "Notification":
                ...
                if event_type == "Bounce":
                    ...
                    bounced_recipients = get_permanent_bounced_emails_from_bounce_obj(
                        bounce_obj
                    )
